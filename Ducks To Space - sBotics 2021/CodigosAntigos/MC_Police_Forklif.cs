...
void Gangorra(){
    if(inclina() >= 330 && inclina() <= 345){
        ...
        while(true){
            ...
            if(inclina() > 1 && inclina() < 10){
                
                motor(-300,-300);
                delay(300);

                while(inclina() >= 1 && inclina() < 10){
                    motor(100,100);
                }
                break;
            }
...
void InitResgate(){
    ...
    //!VERIFICAR RESGATES
        ...
        if(!resgatePos3 && !resgatePos1){
            ...
            //verificação de resgates
            while(true){ 
                //ir pra frente
                motor(300,300); 

                //verif vitima no lado
...
void ResgateDeRéContinua(){
    ...
    while(numeroBolinhas < 3){
        ...
        if(sonar(1) < BallDistance2 && sonar(1) > 17){
            ...
            if(bc.HasVictim()){
                ...
                if(sonar(0) > 9000){
                    ...
                    while(sonar(0) < 85){
                        motor(-300,-300); //?velocidade ajustável (se afastar da parede)
                    }
        ...
        if((bc.touch(0) && sonar(0) > WallDistance2) || sonar(0) > WallDistance2){
...
void SaindoDoResgate(){
    ...
    if(saidaPos1){
        ...
        if(resgatePos3){
            ...
            while(sonar(0) <= WallDistance1 - 20 ){
                motor(-300,-300);
            }
            rot(1000, 80); 
            CorreçãoDeGiroReto();
...
void SaindoDoResgateFaltandoBolinha(){
    print(3, "Tenho q ir embora faltando balls");
    tempoDeGap = 2000;
